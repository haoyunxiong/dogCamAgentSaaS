# Prompt 14: 前端 UI 改造计划

复制给“前端 UI 与交互改造会话”。

```text
你现在执行“租赁商家运营系统”的前端 UI/UX 改造计划。

当前项目根目录：
/Users/lishuangshuang/Documents/租赁SAAS系统

本次只做计划，不修改文件。

必须读取：
1. AGENTS.md
2. docs/xiaogou-saas-roadmap/00_README_总控入口.md
3. docs/xiaogou-saas-roadmap/03_DESIGN_SYSTEM_OVERVIEW_设计系统总纲.md
4. docs/xiaogou-saas-roadmap/phases/phase-02-design-system/artifacts/final-ui-style-lock.md
5. docs/xiaogou-saas-roadmap/phases/phase-02-design-system/artifacts/frontend-implementation-brief.md
6. docs/xiaogou-saas-roadmap/phases/phase-02-design-system/artifacts/page-component-mapping.md
7. docs/xiaogou-saas-roadmap/phases/phase-02-design-system/artifacts/mobile-ui-guidelines.md
8. docs/xiaogou-saas-roadmap/design-references/FINAL_UI_REFERENCE_ONLY.md
9. docs/xiaogou-saas-roadmap/design-references/final-ui-reference-locked/

请扫描当前 Electron / Vue 前端结构，输出前端 UI 改造计划。

计划必须包含：
1. 当前前端结构；
2. 当前页面与最终设计页面的映射；
3. 需要新增/重构的 Base Components；
4. Design Tokens 落地方式；
5. PC 端改造顺序；
6. 手机端改造顺序；
7. 每一步涉及文件；
8. 哪些先 mock 数据；
9. 哪些后接真实业务；
10. 验收标准；
11. 风险点。

禁止：
- 不修改业务代码；
- 不修改 Electron main / Python / IPC / 数据库；
- 不删除 legacy 闲鱼逻辑；
- 不 git add / commit / push。
```
