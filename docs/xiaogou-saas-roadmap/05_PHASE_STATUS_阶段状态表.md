...
当前状态：Phase 04 本地完整 Demo 收口版完成 / 阶段 3D、阶段 3 收口、阶段 4、阶段 5、阶段 6 最小 Demo 已接入 / 本地配置中心 stores/config 非敏感配置读取保存已接入 / 已开放 5 个 gated internal write operationTypes / 外部 real API 与 rollback 仍关闭
...
- [x] 阶段 3D 已完成：`xianyu.order.sync` 闲鱼 mock/sandbox payload preview-only 网关已接入，真实闲鱼同步仍 disabled
- [x] 阶段 3 收口已完成：顺丰 / 免押 / 闲鱼 provider preview-only 能力统一验收，real mode 全 disabled
- [x] 阶段 4 最小 Demo 已完成：本地 actor / role / merchant / store context 与 permission gating 接入 safeOps
...
- 阶段 3D 已完成 `xianyu.order.sync` 闲鱼 mock/sandbox preview-only；
...
- 当前 `xianyu.order.sync`：mock / sandbox 只生成脱敏 payload preview 与 mock sync / sandbox payload shape，不发送 HTTP，不写 `rental_orders`，不更新订单状态；
- 阶段 3 不是直接开放真实外部调用；
- 后续必须先 mock / sandbox，再经二次确认、external idempotency、external audit 和失败补偿后，才允许单独评估 real 接入。

阶段 4 / 5 / 6 本地 Demo 收口结论：

- actor context：本地 Demo 支持 `owner`、`operator`、`viewer` 三种角色；
- merchant / store context：safeOps request 与 audit metadata 携带 `merchantId`、`storeId`、`actorId`、`role`；
- permission gating：`viewer` 只读，不能 execute；`operator` 可执行阶段 1/2 内部安全操作；`owner` 可执行内部安全操作并查看外部 preview；
- external real：所有外部 provider 的 real mode 与 external execute 仍 disabled；
- health readiness：Settings / Dashboard 展示 DB、safeOps tables、external gateway、rollback 与 gated operations 状态；
- config center：Settings / Dashboard 读取本地 MySQL `stores` / `config`，只允许保存门店与非敏感白名单配置；敏感凭证仅展示状态，不返回明文；
- Demo 产品闭环：Dashboard、Orders、Devices、Schedule、Logistics、Deposit、Settings 可连续查看本地 Demo 能力边界；
- 生产上线仍未开启，后续必须单独做 staging / production 分离、备份回滚演练、日志脱敏和 release candidate checklist。
...
- preview-only operationType：`xianyu.order.sync`；
- provider：`xianyu_platform`；
- 默认 mode：`disabled`；
- `mode=mock`：只生成 mock sync preview，不真实同步闲鱼订单；
- `mode=sandbox`：只生成 sandbox payload preview，不发送请求；
- `mode=real`：必须返回 disabled / blocked；
- `execute`：必须返回 `SAFE_OP_EXTERNAL_DISABLED`；
- `externalCallWillExecute=false`、`writeWillExecute=false` 必须始终成立；
- preview 可写 audit / confirm token hash / idempotency / rollback placeholder；
- 禁止 HTTP / fetch / axios / request / Python / 闲鱼 SDK；
- 禁止写 `rental_orders`、订单状态、设备、档期、物流、免押或任何外部平台；
- 敏感字段必须脱敏，不输出手机号、地址、姓名、商品标题、cookie、session、token、API key 明文；
- rollback executor 仍为 unavailable。
