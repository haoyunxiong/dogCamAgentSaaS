...
- migration SQL / rollback SQL / protected runner 已准备，本地测试库已 apply；
- persistence / crypto / repository 已接入本地 safeOps 表；
- preview 可写 audit / confirm token hash / idempotency key / rollback placeholder；
...
- 本地测试库 `xianyu_agent` 已 apply；
- safeOps persistence DB wiring 已完成；
- 生产库 migration 禁止执行。
...
4. migration scaffolding + protected runner；（已完成）
5. persistence / crypto / repository DB wiring；（已完成）
