...
| `xianyu_agent` MySQL 库名 | `scripts/mysql_schema.sql`, `electron/main/mysqlConfig.js`, `python/config_manager.py` | 数据库命名强绑定 | 只记录；改库名属于高风险迁移 |
| `XIANYU_*` 环境变量 | `electron/main/index.js`, `electron/main/mysqlConfig.js`, `python/bridge.py`, `scripts/start_xianyu_desktop.sh` | 环境变量强绑定 | 后续可引入兼容别名，不可直接删除 |
| `start_xianyu_desktop*` 脚本 | `scripts/` | 启动脚本强绑定 | 启动流程关键路径，暂不动 |
...
| localStorage key `xianyu.deposit*` | `depositApiService.js`, `depositMockService.js`, `depositSettingsService.js` | 命名 legacy | 免押能力保留，key 后续兼容迁移 |

## 自动回复 Agent 强绑定

| 模块 | 位置 | 类型 | 建议 |
|---|---|---|---|
| classify/price/tech/default agent | `python/XianyuAgent.py`, `Prompts.vue`, `DEFAULT_PROMPTS` | 可降级为 legacy | 不作为新产品核心，可转话术助手 |
| 人工接管模式/切换关键词 | `python/main.py`, `TakeoverBoard.vue`, `session_reply_state` | 可降级为 legacy | 可转为员工待办，但先保留 |
...
- `xianyu_agent` 数据库名，因为改库名涉及迁移、备份、回滚和 launcher/env 同步。
