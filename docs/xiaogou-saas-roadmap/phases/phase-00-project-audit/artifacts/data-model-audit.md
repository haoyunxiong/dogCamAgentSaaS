...
当前项目数据层存在“文档描述 SQLite、代码现实 MySQL + 少量 SQLite 聊天历史”的不一致。

- Electron 主业务数据：当前主要通过 `electron/main/mysqlAdapter.js` 访问 MySQL。
- Python 主业务数据：通过 `python/mysql_helper.py` 访问 MySQL，并用 wrapper 提供 sqlite-like API。
- Python 聊天历史：仍有 `chat_history.db` / `context_manager.py` 相关 SQLite 逻辑。
- `DATABASE.md` 仍描述 `app_config.db` SQLite 为主库，已经落后于当前代码现实。

Phase 00 不改 schema，不执行迁移。

## 数据访问入口

| 位置 | 当前作用 | 风险 |
|---|---|---|
| `electron/main/dbManager.js` | Electron 主进程主要 DAO / service 聚合 | 文件很大，业务和迁移逻辑混合 |
| `electron/main/mysqlAdapter.js` | MySQL 连接池和 `get/all/run/exec/transaction` | 采用 async，但注释仍说模拟 better-sqlite3 |
...
| `python/config_manager.py` | Python 配置管理，连接 `xianyu_agent` | 命名 legacy |
| `python/context_manager.py` | 聊天历史、商品缓存、部分 item cache | 混用 MySQL item cache 和 SQLite chat history |
| `python/services/rental_repository.py` | Python 租赁服务 repository | MySQL 业务表 + SQLite chat history |
| `DATABASE.md` | 旧数据库说明 | 与当前现实不一致，需要后续更新 |
...
| 字段/表 | 现状 | 判断 |
|---|---|---|
| `stores` | `dbManager.js` 会确保默认店铺 `xiaogou`、`kuaimen` | 有门店雏形 |
...
- `expense_records`
- `inquiry_events`
- `rental_pricing`（如果不同门店价格不同）
- `deposit_exemption_rules`（如果不同门店规则不同）

### 渠道维度候选字段

以下字段未来不应只叫 `item_id` / `chat_id`：

- `item_id` -> `channel_item_id` 或 `source_item_id`
- `chat_id` -> `channel_conversation_id`
- `session_id` -> 需要区分 `channel_session_id` / `internal_session_id`
...
- `dbManager.js` 启动时仍执行 `ensureColumn`、`CREATE TABLE IF NOT EXISTS` 和部分 `ALTER TABLE`，这会让 schema 变化分散在代码里。
- `scripts/mysql_schema.sql` 和 `dbManager.js` 中的建表/补列逻辑可能不是完全同步。
- `DATABASE.md` 仍写 SQLite 主库，可能误导后续执行者。
- 库名、环境变量、备份文件名都仍带 `xianyu`，未来重命名不能直接改，必须迁移。
